    ...
    public partial class Form2 : Form
    {
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            //    {
                ...
                Excel.Range columnARange = worksheet.Range["A:A"];              
                Excel.Range nonEmptyCells = columnARange.SpecialCells(Excel.XlCellType.xlCellTypeConstants);                
                int count = nonEmptyCells.Cells.Count;
                //int count = range.Rows.Count;

                for (int row = 2; row <= count; row++)
                {

                var checkEmpty = ((Microsoft.Office.Interop.Excel.Range)range.Cells[row, 1]).Value2;
                 if (checkEmpty == null || string.IsNullOrEmpty(checkEmpty.ToString()))
                    break;
